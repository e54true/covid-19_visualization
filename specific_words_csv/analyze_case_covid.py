...
# Reading CSV files
df1 = pd.read_csv('twitter_covid_results.csv')
...
ax1.set_ylabel('Covid Count', color='tab:blue')
ax1.plot(merged_df['Month'], merged_df['Covid Count'], color='tab:blue')
...
plt.title('Trend Comparison with covid and case')
plt.show()

# Calculating correlation coefficient (optional)
correlation = merged_df['Covid Count'].corr(merged_df['counts'])
print(f'Correlation between covid Count and Cases Count: {correlation}')